...
def cwt_and_viz(x, ts, show_max_rows=False):
    ...
    # S1 = ifft(U1_f * ts.phi_f[0])
    # U2_f = fft(U2, axis=-1)
    # S2 = ifft(U2_f * ts.phi_f[0][None, None])
    ...
    if show_max_rows:
        ...
        plot(t, U1[mx_idx], title=title, ylims=(0, None), show=1,
             ylabel="A.M. rate [Hz]", xlabel="time [sec]")
...
freqs = [650, 700]
x = np.sum([np.cos(2*np.pi * f * t) for f in freqs], axis=0)
from scipy.signal.windows import tukey
w = tukey(512 + 256)
w = np.pad(w, [769-128, 768-128])
x *= w

title="$\cos(2\pi {} t) \cdot (1 + \cos(2\pi {} t))/2$".format(f0, f1)
plot(t, x, show=1, title=title, xlabel="time [sec]")
#%%
cwt_and_viz(x, ts, show_max_rows=True)
...
#%%
cwt_and_viz(x, ts, show_max_rows=False)
